...
const DrawingScreenSimple = ({ navigation, route }) => {
  ...
  const predictCharacter = async (uri) => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/predictsimple', {
        method: 'POST',
        body: formData,
        headers: {
      ...
      navigation.navigate("PlayGroundSimple", { drawnCharacter: data.predicted_character, wordIndex });
...
export default DrawingScreenSimple;
