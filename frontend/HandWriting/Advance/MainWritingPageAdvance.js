...
export default function MainWritingPageAdvance({ item, navigation }) {
  ...
  const getCategoryVideo = async (category) => {
    const db = firebase.firestore();
    const videoRf = db.collection("writingdataAdvance");
  ...
  const handleNext = () => {
    navigation.navigate("PlayGroundAdvance", { category });
  };
